"""
 ...
 image_retrieval.py  (author: Fang Chen / git: ankonzoid)
...
# construct a path for output directory
...
E_train = model.predict(X_train) # this output the embedding for all input images
E_train_flatten = E_train.reshape((-1, np.prod(output_shape_model))) # squeeze the images into 1 dimension
...
#-------------- use learned embbeding to do clustering with KNN.

# Fit kNN model on training images
print("Fitting k-nearest-neighbour model on training images...")
knn = NearestNeighbors(n_neighbors=5, metric="cosine")
knn.fit(E_train_flatten) # use KNN to cluster all image embeddings

# Perform image retrieval on test images
print("Performing image retrieval on test images...")
for i, emb_flatten in enumerate(E_test_flatten):
    _, indices = knn.kneighbors([emb_flatten]) # given a test image embedding, find which class is mostly close to
    img_query = imgs_test[i] # get the real image
    imgs_retrieval = [imgs_train[idx] for idx in indices.flatten()] # get the similar image from training set
    outFile = os.path.join(outDir, "{}_retrieval_{}.png".format(modelName, i))
    plot_query_retrieval(img_query, imgs_retrieval, outFile) # see how query and the selected training image match up

#-------------- use learned embbeding for tsne
# Plot t-SNE visualization
print("Visualizing t-SNE on training images...")
outFile = os.path.join(outDir, "{}_txssne.png".format(modelName))
